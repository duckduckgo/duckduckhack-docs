...
- ### `minTopicsForMenu` *integer* [optional]
	...
	The topics in the header (and their superscripts) are generated by setting the property `answerItemTopic` on each item (for example, using the `normalize` function.) The topics header will show if, across all items in this Instant Answer, there are more unique values than `minTopicsForMenu`.
